...
class DigiCamControlService extends ICameraProvider {
  ...
  async downloadPhoto(filename, { deadline = Date.now() + 8000 } = {}) {
    ...
    while (Date.now() < deadline && !this.disposed) {
      const candidates = basename
        ? [`/image/${encodeURIComponent(basename)}`, `/preview.jpg?t=${Date.now()}`]
        : [`/preview.jpg?t=${Date.now()}`];

      for (const suffix of candidates) {
        try {
          const res = await this._fetch(suffix, { timeout: 8000, raw: true });
          if (res.ok && isCompleteJpeg(res.buffer)) {
            return { ok: true, buffer: res.buffer, filename: basename, source: suffix };
          }
          lastError = res.ok
            ? 'File JPG belum selesai ditulis'
            : `HTTP ${res.status} saat mengunduh foto`;
        } catch (err) {
   ...
   */
  async capture({ sessionDir, timeoutMs = 10000, pollIntervalMs = 300 } = {}) {
    const targetDir = sessionDir || this.sessionDir;

    const health = await this.healthCheck();
    if (!health.connected) return { ok: false, code: 'NOT_CONNECTED', error: health.error };
    ...
    if (!filename) {
      return { ok: false, code: 'CAPTURE_TIMEOUT', error: 'Kamera tidak merespon, coba lagi' };
    }

    const downloaded = await this.downloadPhoto(filename, { deadline: Date.now() + 8000 });
    if (!downloaded.ok) {
    ...
    const result = {
      ok: true,
      provider: this.name,
      filename: downloaded.filename || path.basename(filename),
      dataUrl: `data:image/jpeg;base64,${downloaded.buffer.toString('base64')}`,
    };
